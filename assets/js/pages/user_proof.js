...
import { videoPreview } from '../components/Modal';
...
export default class LiveInformation extends EventEmitter {
	...
	init(element) {
		let gethasAudit = isAudit();

		gethasAudit.then((data) => {
			this.data.PassAudit = data ? data : {};
			this.data.HasAudit = data ? data.status : 0; // 0.未上传 1.未审核 2.审核通过 3.审核不通过
	...
	_init() {
		...
		this.passVideoEl = this.LiveInformationEl.getElementsByClassName(this.options.passVideoClass);
	...
	_bindEvent() {
		...
		// 视频预览
		if (this.passVideoEl.length > 0) {
			addEvent(this.passVideoEl[0], 'click', () => {
				let href = getData(this.passVideoEl[0], 'href');
